...
async def GPTarot_request(client, prompt:str) -> str:
    for result in client.send_message("chinchilla", prompt, timeout = 60): # chinchilla is the name of the engine ChatGPT3.5-Turbo
        pass
    return result["text"]
...
async def generatePrompt(client, data:dict) -> str:
    ...
    answer = await GPTarot_request(client, prompt)
    return answer